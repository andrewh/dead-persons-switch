...
const triggerPathRegExp = new RegExp(`^/trigger/(${triggerNamePattern})/with/key/(${keyPattern})$`);
...
if (process.env.SECRET == null) throw new Error('Must define the SECRET environment variable');

if (process.env.PORT == null) throw new Error('Must define the PORT environment variable');

if (process.env.TRIGGERS == null) throw new Error('Must define the TRIGGERS environment variable');

if (process.env.WEBHOOK_URL == null) throw new Error('Must define the WEBHOOK_URL environment variable');
...
if (configRegExp.test(process.env.TRIGGERS)) throw new Error('TRIGGERS must be "foo:1000,bar:5000:6:10000,[...]"');
...
process.env.TRIGGERS.split(',').forEach(tuple => {
  ...
  if (triggers[triggerName] != null) throw new Error(`Duplicate event ${triggerName}`);
...
const server = http
  .createServer((req, res) => {
    ...
    if (req.method === 'POST') {
      ...
      if (matches == null) return void respondWith(400, `Cannot POST ${reqPath}`);

      const triggerName = matches[1];
      const secretKey = matches[2];

      if (secretKey !== process.env.SECRET) return void respondWith(403, 'Forbidden');

      if (triggers[triggerName] == null) return void respondWith(404, 'There is no such trigger');

      triggers[triggerName].lastTriggerEpoch = Date.now();
      log(`Triggered ${triggerName} @ ` + `${new Date(triggers[triggerName].lastTriggerEpoch)}\n`);
      return void respondWith(200, `Congratulations! You've fired the ${triggerName} event`);
    } else if (req.method === 'GET') {
      const secretKey = url.parse(req.url, true).query.key;

      if (secretKey == null) return void respondWith(200, "Your Dead Person's Switch is working");
...
const notifier = setInterval(() => {
  for (triggerName in triggers) {
    ...
    if (
      trigger.notificationAttemptsRemaining > 0 &&
      (trigger.lastNotificationEpoch == null ||
        Date.now() - trigger.lastNotificationEpoch > trigger.notificationInterval)
    ) {
      ...
      switch (notificationIndex) {
        ...
        case 3:
          ...
          break;
        default:
          description = `${notificationIndex}th`;
      }

      if (trigger.notificationAttemptsRemaining === 0) description += ' and final';
      ...
      POSTJSON(
        ...
        (err, res) => {
          if (err) {
            log(`Failed to notify webhook about ${triggerName} ` + `@ ${new Date()}: ${err.message}\n`);
            trigger.lastNotificationStatus = err.message;
          } else {
            log(`Notified webhook about ${triggerName} @ ` + `${new Date()}\n`);
